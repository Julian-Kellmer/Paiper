...
export function DailySummary({ data }: DailySummaryProps) {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case "clipboard":
        return (
          <ClipboardIcon className="h-5 w-5 dark:text-gray-300" />
        );

      case "dollar":
        return (
          <DollarSignIcon className="h-5 w-5 dark:text-gray-300" />
        );
      ...
      {data.map((item, index) => (
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col items-end" id={`u4gh78_${index}`}>
              <div
