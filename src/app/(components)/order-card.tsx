...
export function OrderCard({ order }: { order: OrderProps }) {
  ...
  const [editedOrder, setEditedOrder] = useState<OrderProps>({ ...order });
  const [editedItems, setEditedItems] = useState<OrderItem[]>([...order.items]);
  const [editedNotes, setEditedNotes] = useState<OrderNote[]>([...order.notes]);
  ...
  const getStatusBadge = () => {
    switch (order.status) {
      ...
      case "preparation":
        return (
          <span
            className="px-2 py-1 text-xs font-medium rounded-full bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400"
          >
            En Preparación
          </span>
        );
      ...
      case "ready":
        return (
          <span
            className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400"
          >
            Listo
          </span>
        );
      ...
      case "completed":
        return (
          <span
            className="px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400"
          >
            Completado
          </span>
        );
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-0">
        ...
        <div className="p-4 border-b">
            ...
            {order.items.map((item, index) => (
              ...
              >
                <div id={`8x6wqo_${index}`}>
                  <span className="font-medium" id={`eqp0pn_${index}`}>
                    {item.quantity}x
                  </span>{" "}
                  {item.name}
                </div>
                <div className="font-medium" id={`q6gqta_${index}`}>
          ...
          <div
            className="mt-4 pt-2 border-t flex justify-between font-bold"
          >
            <div>Total</div>
            ...
            <div className="space-y-4">
              ...
              {editedItems.map((item, index) => (
                <div
                  ...
                  id={`329ufv_${index}`}
                >
                  <div className="flex justify-between" id={`to80vr_${index}`}>
                  ...
                  <div
                    ...
                    id={`s6n8lt_${index}`}
                  >
                    <div className="col-span-7" id={`1icr0m_${index}`}>
                      <Input
                    ...
                    <div className="col-span-2" id={`8872e8_${index}`}>
                      <Input
                        type="number"
                        placeholder="Cant."
                        value={item.quantity}
                    ...
                    <div className="col-span-3" id={`mw6lpb_${index}`}>
                      <Input
                        type="number"
                        placeholder="Precio"
                        value={item.price}
            ...
            <div className="space-y-4">
              ...
              {editedNotes.map((note, index) => (
                <div
                  ...
                  id={`ej08za_${index}`}
                >
                  <div className="flex justify-between" id={`1zhj8p_${index}`}>
                  ...
                  <div className="space-y-2" id={`whk6lj_${index}`}>
                    <Select
                      value={note.type}
                      onValueChange={(value) =>
                    ...
                    >
                      ...
                      <SelectContent id={`m6aca4_${index}`}>
                        <SelectItem value="allergy" id={`itnx2k_${index}`}>
                          Alergia
                        </SelectItem>
                        <SelectItem value="preference" id={`tb2is5_${index}`}>
                          Preferencia
                        </SelectItem>
                      </SelectContent>
                    </Select>
