	...
	useEffect(() => {
		const fetchProfitabilityData = async () => {
			try {
				...
				const { data: todayOrders, error: todayError } = await supabase.from('orders').select('*, order_items(*)').gte('created_at', today.toISOString()).lt('created_at', tomorrow.toISOString());

				if (todayError) throw todayError;

				// Fetch week's orders
				const { data: weekOrders, error: weekError } = await supabase.from('orders').select('*, order_items(*)').gte('created_at', weekStart.toISOString()).lt('created_at', weekEnd.toISOString());
		...
		orders.forEach((order) => {
			...
			if (order.order_items && order.order_items.length > 0) {
				order.order_items.forEach((item: any) => {
					...
					orderCosts += (item.quantity || 0) * (item.cost || 0);
				});
			}
			...
			if (chartDataMap.has(key)) {
				...
				chartDataMap.set(key, {
					...
					costs: orderCosts,
					profit: (order.total || 0) - orderCosts,
				});
			}
		});
